...
function drawProp(ctx: CanvasRenderingContext2D, kind: string, variant: number, rng: Rng): void {
  switch (kind) {
    ...
    case "windmill": {
      // American farm windmill: a lattice tower under a face-on multi-blade
      // fan wheel, with a tail vane off to one side.
      const baseHalf = 3.4;
      const topHalf = 1.1;
      const topY = -5;
      const baseY = 9;
    ...
    case "tractor": {
      // Classic farm tractor: big rear wheel, small front wheel, red body.
      ctx.fillStyle = "#c0433a";
      ctx.beginPath();
      ctx.moveTo(-8, 6);
      ctx.lineTo(-8, 2.5);
